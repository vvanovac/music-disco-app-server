...
const LENGTH = 64;
export const comparePasswords = (input: string, match: string, salted: string): Promise<boolean> => {
  return new Promise<boolean>((resolve: (data) => void, reject: (data) => void) => {
    crypto.pbkdf2(input, salted, 10000, LENGTH, digest, (err: Error, hash: Buffer) => {
      if (err) {
        reject(err);
      } else {
...
export const hashPassword = (input: string): Promise<IHashedPassword> => {
  return new Promise<IHashedPassword>((resolve: (data) => void, reject: (data) => void) => {
    crypto.pbkdf2(input, salt, 10000, LENGTH, digest, (err?: Error, hash?: Buffer) => {
      if (err) {
        reject(err);
      } else {
